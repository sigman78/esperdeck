...
{
    /* touch.ini and font.ini were absent from the old hardcoded reset list
     * — an oversight, fixed by this registration: factory reset now clears
     * every shell setting. */
    storage_reset_register(APP_FX_INI);
    storage_reset_register(APP_SAVER_INI);
    storage_reset_register(APP_TOUCH_INI);
    storage_reset_register(APP_FONT_INI);
}
